// NetApp Enterprise Configurator & Code Generator Engine (v1.9.1)
    ...
    if (state.mode === "greenfield") {
      ...
      if (shelfCardSizing.overflow) {
        errors.push({
          ...
          msg: `${state.sizing.shelfType} Shelves (${shelfCount} per HA pair): ${state.sizing.controller} can direct-attach at most ${shelfCardSizing.maxDirectAttachShelves} ${state.sizing.shelfType} shelves per HA pair (1 onboard stack plus ${shelfCardSizing.slotsAvailable} PCIe expansion slot(s)).`,
          ...
          fix: `Reduce Disk Count on Step 1 to ${shelfCardSizing.maxDirectAttachShelves * getShelfBayCount(state.sizing.shelfType)} or fewer disks per node pair, or use a storage switch fabric topology to scale beyond onboard/PCIe direct-attach limits.`
        });
      }
