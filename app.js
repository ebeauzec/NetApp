...
function recalculateCapacity() {
  ...
  const rawTotalParityDisks = parityDisksPerAggr * 2;

  const rawTotalSpareDisks = state.sizing.spareDisks * (state.sizing.nodeCount / 2) * 2;
...
function getExpansionCardsAndPorts(model, shelfType, shelfCount) {
  ...
  // beyond the single onboard stack every platform has.
  result.maxDirectAttachShelves = stackSize * (slotPriority.length + 1);

  if (cardsNeeded <= 0) {
    return result;
  }

  // Never invent a slot number beyond what this platform actually has -- a config that
  // needs more cards than physical slots is a real overflow (flagged via result.overflow),
  // not something to paper over with a fabricated slot number.
  result.overflow = cardsNeeded > slotPriority.length;
    ...
    if (state.mode === "greenfield") {
      ...
      if (shelfCardSizing.overflow) {
        errors.push({
          ...
          fix: `Reduce Disk Count on Step 1 to ${shelfCardSizing.maxDirectAttachShelves * 24} or fewer disks per node pair, or use a storage switch fabric topology to scale beyond onboard/PCIe direct-attach limits.`
        });
      }
