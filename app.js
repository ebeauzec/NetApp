// NetApp Enterprise Configurator & Code Generator Engine (v1.7.2)
...
function generateCablingRows() {
  ...
  if (isSg) {
    ...
    if (state.metrocluster && state.metrocluster.enabled) {
      ...
      // every pair is guaranteed its own dedicated shelf cabling entry, even when the configured
      // disk count yields fewer physical shelves per site than there are HA pairs -- pairs beyond
      // shelfCount get the NEXT shelf number (Shelf A2, A3, ...), never reusing a shelf another
      // pair already occupies: a single NS224 shelf has only 4 physical storage ports (2 per NSM),
      // fully consumed by ONE HA pair's redundant multipath wiring, so two independent HA pairs
      // can never physically share one shelf. validateForm() separately flags this under-
      // provisioned case so the user knows to add disks/shelves, rather than the diagram silently
      // implying more shelves exist than the sizing calculator's shelfCount says were ordered.
      ...
      for (let pairIdx = 1; pairIdx <= numPairs; pairIdx++) {
        ...
        const shelfStart = (pairIdx - 1) * shelvesPerPair;
        const shelvesForThisPair = Math.min(shelvesPerPair, Math.max(1, shelfCount - shelfStart));

        for (let k = 0; k < shelvesForThisPair; k++) {
          const rawShelfIdx = shelfStart + k;
          const s = rawShelfIdx + 1;
      ...
      // 2. Storage Cabling (shelves distributed across HA pairs; every pair gets its own
      // dedicated shelf cabling entry, even if the configured disk count yields fewer physical
      // shelves than HA pairs -- pairs beyond shelfCount get the NEXT shelf number (Shelf 2, 3,
      // ...), never reusing a shelf another pair already occupies: a single NS224 shelf has only
      // 4 physical storage ports (2 per NSM), fully consumed by ONE HA pair's redundant multipath
      // wiring, so two independent HA pairs can never physically share one shelf. validateForm()
      // separately flags this under-provisioned case.
...
function validateForm() {
  ...
  if (state.platform === "ontap") {
    ...
    if (state.mode === "greenfield") {
      ...
      // Insufficient shelves for the configured HA pair count [NEW]. A single shelf's 4
      // physical storage ports (2 per NSM) are fully consumed by one HA pair's redundant
      // multipath wiring -- two independent HA pairs can never physically share one shelf, so
      // the cabling diagram assigns each pair beyond shelfCount its own additional shelf number
      // rather than an impossible shared connection. This warning explains that gap.
      const effectiveNodeCount = parseInt(state.sizing.nodeCount) || 2;
      const pairsNeeded = (state.metrocluster && state.metrocluster.enabled)
        ? Math.max(1, Math.ceil((effectiveNodeCount / 2) / 2)) * 2 // per-site pairs, both sites
        : Math.max(1, Math.ceil(effectiveNodeCount / 2));
      if (state.platform === "ontap" && pairsNeeded > Math.max(1, shelfCount)) {
        warnings.push({
          step: 1,
          title: "Insufficient Shelves for HA Pair Count",
          msg: `Disk Count (${state.sizing.diskCount}) provisions ${Math.max(1, shelfCount)} shelf(es), but ${effectiveNodeCount} nodes need ${pairsNeeded} shelf(es) minimum -- one dedicated shelf per HA pair.`,
          why: "Each NS224 shelf's 4 physical storage ports are fully consumed by a single HA pair's redundant multipath wiring. Separate HA pairs cannot share a shelf, so the cabling diagram now shows extra shelves beyond what the current Disk Count implies.",
          fix: `Increase Disk Count on Step 1 so each HA pair gets its own shelf (at least ${pairsNeeded * 24} disks for ${pairsNeeded} shelf(es) of a 24-slot NS224), or reduce Node Count.`
        });
      }
