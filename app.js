// NetApp Enterprise Configurator & Code Generator Engine (v1.9.2)
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Show the cached "Check for Updates" result (if any) from a previous session
  renderCachedUpdateBadge();
...
// (tools/update_server.py, 127.0.0.1:8766) -- never the wider internet. A page
// loaded from file:// (or any origin) can't fetch docs.netapp.com/endoflife.date
// directly and read the response for a version comparison anyway: docs.netapp.com
// sends no Access-Control-Allow-Origin header, so the browser blocks reading a
// cross-origin fetch() response even though the request itself would succeed.
// The local helper has a normal network stack, re-uses the harvest/drift-check
// tooling documented in DATA_SOURCES.md, then serves the result back over
// localhost with CORS headers it controls. If the helper isn't running, this
// fails fast and tells the user how to start it -- it never silently retries or
// falls back to fetching anything itself.
const UPDATE_HELPER_URL = "http://127.0.0.1:8766";
// Caches the last "Check for Updates" result in localStorage so it survives page reloads --
// the app can show "last verified on <date>" without needing the helper/network every time,
// and it stays exactly as-is until the next successful check overwrites it.
const UPDATE_CACHE_KEY = "netapp_configurator_update_check_cache";
...
function loadUpdateCheckCache() {
  try {
    ...
    console.warn("Failed to read cached update-check result:", e);
    return null;
  }
}

function saveUpdateCheckCache(data) {
  try {
    localStorage.setItem(UPDATE_CACHE_KEY, JSON.stringify(data));
  } catch (e) {
    console.warn("Failed to cache update-check result:", e);
  }
}

function formatUpdateCheckTimestamp(iso) {
  try {
...
function renderCachedUpdateBadge() {
  const badge = document.getElementById("lastUpdateCheckBadge");
  if (!badge) return;
  ...
  badge.style.color = cached.clean ? "var(--color-success)" : "var(--color-warning)";
  badge.innerHTML = `<i data-lucide="${cached.clean ? "check-circle" : "alert-triangle"}" style="width:11px;height:11px;"></i> Last verified ${formatUpdateCheckTimestamp(cached.checkedAt)}`;
  safeCreateIcons();
}

function renderUpdateResultsHtml(data) {
  if (data.clean) {
    return `
      <p style="color: var(--color-success);"><i data-lucide="check-circle" style="width:14px;height:14px;vertical-align:-2px;"></i> No drift detected. This app's version lists are current.</p>
      <ul style="padding-left: 1.2rem; margin-top: 10px;">
        <li>ONTAP: shipping <strong>${data.shippedOntapLatest || "?"}</strong> as latest, endoflife.date reports <strong>${data.liveOntapLatest || "?"}</strong></li>
        <li>StorageGRID: shipping <strong>${data.shippedStoragegridLatest || "?"}</strong> as latest, docs.netapp.com reports <strong>${data.liveStoragegridLatest || "?"}</strong></li>
      </ul>
      <p style="font-size:0.78rem; margin-top:0.75rem;">${data.note || ""}</p>
    `;
  }
  const rows = (data.drift || []).map(d => `<li><strong>${d.field}</strong>: ${d.issue}</li>`).join("");
  return `
    <p style="color: var(--color-warning);"><i data-lucide="alert-triangle" style="width:14px;height:14px;vertical-align:-2px;"></i> Drift detected on ${(data.drift || []).length} field(s):</p>
    <ul style="padding-left: 1.2rem;">${rows}</ul>
    <p style="font-size:0.78rem; margin-top:0.75rem;">${data.note || ""} Nothing was changed automatically -- update ONTAP_VERSIONS / STORAGEGRID_VERSIONS in app.js by hand, per DATA_SOURCES.md's Update Procedure, then rebuild the offline bundle.</p>
  `;
}

async function checkForUpdates() {
  const modal = document.getElementById("updateResultsModal");
  const content = document.getElementById("updateResultsContent");
  if (!modal || !content) return;

  const cached = loadUpdateCheckCache();
  const cachedNotice = cached && cached.checkedAt
    ? `<p style="font-size:0.78rem; color: var(--text-muted); border-bottom: 1px solid var(--glass-border); padding-bottom: 10px; margin-bottom: 10px;">Cached result from ${formatUpdateCheckTimestamp(cached.checkedAt)}, shown below while a fresh check runs:</p>${renderUpdateResultsHtml(cached)}`
    : "";
  content.innerHTML = `${cachedNotice}<div style="text-align:center; padding: 1rem 0;">Checking local update helper and re-fetching ONTAP/StorageGRID reference data (~10s)...</div>`;
  modal.style.display = "flex";
  safeCreateIcons();

  let resp;
  try {
    const fetchOpts = (typeof AbortSignal !== "undefined" && AbortSignal.timeout) ? { signal: AbortSignal.timeout(30000) } : {};
    ...
    content.innerHTML = `
      ...
      <p>This app stays fully offline-capable by design -- it never reaches the internet on its own. To check whether the ONTAP/StorageGRID version lists are still current:</p>
      <ol style="padding-left: 1.2rem;">
        <li>Close this window.</li>
        <li>Next time, start the app via <code style="background:rgba(255,255,255,0.08); padding:2px 6px; border-radius:4px;">launch.bat</code> (or <code style="background:rgba(255,255,255,0.08); padding:2px 6px; border-radius:4px;">python launch.py</code>) instead of opening the HTML file directly -- it starts this helper for you automatically.</li>
  ...
  if (!resp.ok) {
    ...
    content.innerHTML = `<p style="color: var(--color-error);">Update helper returned an error${errText ? ": " + errText : "."}</p>`;
    return;
  }

  const data = await resp.json();
  content.innerHTML = renderUpdateResultsHtml(data);
  saveUpdateCheckCache(data);
  renderCachedUpdateBadge();
  safeCreateIcons();
}
...
async function downloadSourceFiles() {
  ...
  const files = [
    ...
    'app.js',
    'bundle_offline.py',
    'tools/harvest_reference_data.py',
    'tools/apply_reference_data.py',
    'tools/update_server.py',
    'launch.py',
    'launch.bat',
    'DATA_SOURCES.md',
    'README.md',
...
function setupEventListeners() {
  ...
  const btnCloseUpdateResults = document.getElementById("btnCloseUpdateResults");
  if (btnCloseUpdateResults) {
    btnCloseUpdateResults.addEventListener("click", () => {
      const modal = document.getElementById("updateResultsModal");
...
function recalculateCapacity() {
  ...
  const diskSizeGb = parseDiskSizeToGb(state.sizing.diskSize);
  const numPairs = state.sizing.nodeCount / 2;
  const totalDisks = state.sizing.diskCount * numPairs;
  const rawGb = totalDisks * diskSizeGb;

  const raidType = state.sizing.raidType;
  const raidGroupSize = state.sizing.raidGroupSize;

  // One aggregate per node (2 per HA pair, matching generateOntapCliCode()/
  // generateAnsiblePlaybook()) -- not a flat 2 aggregates cluster-wide. diskCount is already
  // per-pair, so each aggregate holds the same disksPerAggr regardless of pair count.
  const totalAggregates = state.sizing.nodeCount;
  const disksPerAggr = state.sizing.diskCount / 2;
  ...
  const rawTotalSpareDisks = state.sizing.spareDisks * numPairs * 2;

  // Clamp parity + spares so the visualized capacity segments can never exceed the physical raw capacity
...
function solveDiskCountForUsableCapacity(targetUsableGb) {
  ...
  // Internal bays (if any) are free capacity consumed first; an external shelf type's real
  // max is internal + external combined (see getShelfCount()/updateDiskCountRange()), not
  // external alone.
  const maxDisks = shelfType === "Internal"
    ? (getInternalBayCount(state.sizing.controller) || 24)
    : ((getInternalBayCount(state.sizing.controller) || 0) + getExpansionCardsAndPorts(state.sizing.controller, shelfType, 1).maxDirectAttachShelves * getShelfBayCount(shelfType));
...
function updateDiskCountRange() {
  ...
  if (isSg) {
    ...
    if (shelf === "Internal") {
      ...
      // depends on controller + shelf type, not the disk count being validated. Platforms with
      // real internal bays (getInternalBayCount()) get those added on top -- internal capacity
      // is free/automatic and only the overflow needs this external shelf type (see
      // getShelfCount()), so the combined max is internal + external, not external alone.
      const cardSizing = getExpansionCardsAndPorts(state.sizing.controller, shelf, 1);
      const internalBaysForCap = getInternalBayCount(state.sizing.controller) || 0;
      const maxDisksForPlatform = internalBaysForCap + cardSizing.maxDirectAttachShelves * getShelfBayCount(shelf);
...
// Real per-shelf bay count. DS212C is a 12-bay LFF SAS shelf; NS224 and DS224C are both
// 24-bay. Every shelfCount calculation in the app must divide by this, not a hardcoded 24 --
// hardcoding 24 undercounted DS212C shelves (and therefore cabling/CLI/BOM/PCIe-card sizing)
// by half. See CHANGELOG.md.
function getShelfBayCount(shelfType) {
  return shelfType === "DS212C" ? 12 : 24;
}

// Real internal drive bay count for platforms whose base storage capacity doesn't require
// configuring/cabling a separately-purchased external shelf -- sourced from NetApp's published
// platform specification pages/datasheets. Returns null for platforms not confirmed to have
// this, in which case the app requires at least one real external shelf (existing behavior,
// unchanged).
//
// A70/A90/C80 are included on the strength of NetApp's own hot-add documentation, which
// explicitly states the baseline HA pair "has only internal storage (no external shelves)"
// before any hot-add procedure begins (github.com/NetAppDocs/ontap-systems/_include/
// cable-70-90-hot-add-shelf.adoc), corroborated by a 48-internal-SSD-slot 4U chassis spec.
// A1K is included the same way: its base storage is a 2RU x 24-slot storage chassis that ships
// bundled with the 2RU controller as one purchasable unit, at the same treatment as every other
// platform in this table -- a real, separate NSM/shelf-cabling relationship still exists
// underneath the bundling (its own hot-add doc calls the baseline "1 existing NS224 shelf"),
// but for consistency with the rest of this table it's modeled as available without configuring
// an additional external shelf. A900 and C800 remain excluded: no bundled-chassis or "internal
// storage" language was found for either in available sources. See DATA_SOURCES.md.
function getInternalBayCount(model) {
  ...
  // "A900" contains "A90" and "C800" contains "C80" as substrings -- excluded explicitly so
  // they fall through to `return null` instead of incorrectly matching this branch (same
  // precaution as getControllerPorts()/getRealMaxNs224Shelves() apply for the same pair).
  if ((m.includes("A90") && !m.includes("A900")) || m.includes("A70") || (m.includes("C80") && !m.includes("C800"))) return 48;
  if (m.includes("A1K")) return 24;
...
// for an internal-only configuration, with no further special-casing needed.
//
// For an external shelf type (NS224/DS224C/DS212C) on a platform that also has real internal
// bays (getInternalBayCount()), those internal bays are free capacity consumed first -- only
// the overflow beyond them needs an external shelf at all. Without this, a platform with e.g.
// 48 internal bays selecting NS224 with diskCount=60 would shelve all 60 disks (3 shelves)
// instead of correctly shelving just the 12 that don't fit internally (1 shelf).
function getShelfCount(diskCount, shelfType, model) {
  if (shelfType === "Internal") return 0;
  const internalBays = getInternalBayCount(model) || 0;
  const externalDisks = Math.max(0, diskCount - internalBays);
  if (externalDisks <= 0) return 0;
  return Math.max(1, Math.ceil(externalDisks / getShelfBayCount(shelfType)));
}

// Real NS224 maximum total shelves per HA pair (1 onboard/base + hot-add capacity), sourced
// directly from NetApp's per-platform NS224 hot-add cabling guides (github.com/NetAppDocs/
// ontap-systems, ns224/hot-add-*.adoc and _include/cable-*-hot-add-shelf.adoc) and NetApp's
// Knowledge Base -- NOT derived from the PCIe-slot-count formula in getExpansionCardsAndPorts()
// below, which does not match real shelf-per-slot ratios (that formula implied an AFF A400
// could take 8 NS224 shelves; NetApp's own KB caps it at 2). Returns null for platforms with
// no published per-platform NS224 hot-add guide (currently the FAS lineup), in which case the
// caller falls back to the slot-count formula. See DATA_SOURCES.md.
function getRealMaxNs224Shelves(model) {
  const m = (model || "").toUpperCase();
  // Order matters: longer/more specific codes must be checked before shorter substrings they
  // contain (e.g. "A900" contains "A90", "C800" contains "C80") -- same precaution as
  // getControllerPorts().
  if (m.includes("A900")) return 4;                            // 1 existing + up to 3 additional
  if (m.includes("C800")) return 2;                             // shares the A800/C800 hot-add guide
  if (m.includes("A400") || m.includes("C400")) return 2;       // confirmed via NetApp KB
  if (m.includes("A250") || m.includes("C250")) return 2;       // 1 onboard (e2a/e2b) + 1 hot-add (e1a/e1b)
  if (m.includes("A150")) return 1;                             // SAS-onboard platform; NS224 only via 1 RoCE mezzanine slot
  if (m.includes("A1K")) return 4;                              // 1 existing + up to 3 additional
  if (m.includes("A90") || m.includes("A70") || m.includes("C80")) return 2;
  if (m.includes("A50") || m.includes("A30") || m.includes("C60") || m.includes("C30")) return 3;
  if (m.includes("A20")) return 2;                              // 1 onboard + 1 hot-add
  return null;
}
...
function getExpansionCardsAndPorts(model, shelfType, shelfCount) {
  ...
  const result = {
    cards: [],
    portPairs: [],
    // Real per-platform PCIe slot capacity -- see slotPriority below. Used to detect
    // when a configuration needs more expansion cards than the platform physically has
    // (see validateForm()'s "High Shelf Count for Direct Cabling" check).
    slotsAvailable: 0,
    slotsUsed: 0,
    overflow: false,
    maxDirectAttachShelves: 0
  };
  ...
  // Never invent a slot number beyond what this platform actually has, and never exceed the
  // real sourced max shelf count -- either is a real overflow (flagged via result.overflow),
  // not something to paper over with a fabricated slot number.
  result.overflow = cardsNeeded > slotPriority.length || shelfCount > result.maxDirectAttachShelves;
  const cardsToPlace = Math.min(cardsNeeded, slotPriority.length);
  result.slotsUsed = cardsToPlace;

  const cardModel = isNvme ? "X1148A" : "X1144A";
  const cardDescription = isNvme
    ? "2-Port 100GbE RoCE QSFP28 NVMe Storage HBA"
    : "4-Port 12Gb SAS HBA";

  for (let c = 0; c < cardsToPlace; c++) {
    const slot = slotPriority[c];
    let ports = [];
    if (isNvme) {
...
function getControllerPorts(model) {
  const m = (model || "").toUpperCase();
  // Port names below are sourced from docs.netapp.com install-cable / install-detailed-guide
  // pages per platform (see DATA_SOURCES.md's "Controller Port Catalog" section for exact URLs
  // and the ASA r2 vs AFF hardware-identity confirmation). ASA_* keys share their AFF
  // counterpart's real ports -- confirmed directly against the ASA r2 cabling guides, which
  // show identical port names to the AFF guides for the same physical chassis (A1K/A90/A70/C80,
  // and A20/A30/A50/C60/C30). Order matters below: longer/more specific model codes (A900,
  // C800, A400, C400, A150, A250, C250, A20, FAS70/90) are matched before shorter substrings
  // they contain (e.g. "A900" contains "A90", "C800" contains "C80") to avoid misclassifying them.
  if (m.includes("A900")) {
    // AFF/ASA A900: cluster on PCIe slots 4/8, not onboard -- docs.netapp.com/us-en/ontap-systems/a900/install-detailed-guide.html
    return { cluster: ["e4a", "e8a"], storage: ["e1a", "e1b"], management: "e0M", data: ["e3a", "e3c"] };
  } else if (m.includes("C800")) {
    // AFF/ASA C800 uses the generic onboard cluster/data scheme (unlike A900), with PCIe-slot NS224 storage
    return { cluster: ["e0a", "e0b"], storage: ["e3a", "e3b"], management: "e0M", data: ["e0c", "e0d"] };
  } else if (m.includes("A1K") || m.includes("A90") || m.includes("A70") || m.includes("C80")) {
    // AFF/ASA A1K/A90/A70/C80 share this scheme -- docs.netapp.com/us-en/ontap-systems/a70-90/install-cable.html,
    // .../a1k/install-cable.html, confirmed against docs.netapp.com/us-en/asa-r2/install-setup/cable-hardware-a1k.html
    // and cable-hardware-a70-90.html (ASA r2 uses the identical physical chassis/ports as AFF)
    return { cluster: ["e1a", "e7a"], storage: ["e8a", "e8b"], management: "e0M", data: ["e9a", "e9b"] };
  } else if (m.includes("A400")) {
    return { cluster: ["e0a", "e0b"], storage: ["e2a", "e2b"], management: "e0M", data: ["e0c", "e0d"] };
  } else if (m.includes("C400")) {
    return { cluster: ["e0a", "e0b"], storage: ["e4a", "e4b"], management: "e0M", data: ["e0c", "e0d"] };
  } else if (m.includes("A150")) {
    // AFF/ASA A150's storage HBA uses onboard SAS ports with no "e" prefix (0a/0b), not e0c/e0d
    return { cluster: ["e0a", "e0b"], storage: ["0a", "0b"], management: "e0M", data: ["e0c", "e0d"] };
  } else if (m.includes("A250") || m.includes("C250")) {
    return { cluster: ["e0a", "e0b"], storage: ["e2a", "e2b"], management: "e0M", data: ["e0c", "e0d"] };
  } else if (m.includes("A20")) {
    // AFF/ASA A20: single-I/O-module scheme -- cluster e4a/e4b, not the two-module e2a+e4a scheme A30/A50/C30/C60 use
    return { cluster: ["e4a", "e4b"], storage: ["e3a", "e3b"], management: "e0M", data: ["e2a", "e2b"] };
  } else if (m.includes("A50") || m.includes("A30") || m.includes("C60") || m.includes("C30")) {
    // AFF/ASA A50/A30/C60/C30 share the two-I/O-module scheme -- docs.netapp.com/us-en/ontap-systems/a20-30-50/install-cable.html,
    // .../c30-60/install-cable.html, confirmed against docs.netapp.com/us-en/asa-r2/install-setup/cable-hardware-a20-30-50.html
    // and cable-hardware-c30.html: cluster e2a+e4a (two modules) with host data on e2b/e4b
    return { cluster: ["e2a", "e4a"], storage: ["e3a", "e3b"], management: "e0M", data: ["e2b", "e4b"] };
  } else if (m.includes("FAS90") || m.includes("FAS70")) {
    // FAS70/FAS90 share AFF A70/A90's cluster/host scheme but different NS224 storage slots (10/11, not 8/11)
    // -- docs.netapp.com/us-en/ontap-systems/fas-70-90/install-cable.html
    return { cluster: ["e1a", "e7a"], storage: ["e10a", "e10b"], management: "e0M", data: ["e9a", "e9b"] };
  } else if (m.includes("FAS")) {
    // FAS8300/FAS8700/FAS2820/FAS9500 (generic FAS scheme): onboard SAS storage HBA (0a/0b), no PCIe storage card needed
    return { cluster: ["e0a", "e0b"], storage: ["0a", "0b"], management: "e0M", data: ["e0c", "e0d"] };
  } else {
    // Fallback for any unrecognized model string -- matches the entry-level e0a-e0d onboard scheme
...
function generateCablingRows() {
  ...
  const shelfCount = getShelfCount(state.sizing.diskCount, state.sizing.shelfType, state.sizing.controller);
  ...
  if (isSg) {
    ...
    if (state.metrocluster && state.metrocluster.enabled) {
      ...
      // 2. Storage Cabling (Shelves local to each site). Loops over HA pairs (not shelves) so
      // every pair is guaranteed its own dedicated shelf cabling entries. state.sizing.diskCount
      // is "Disks per Node Pair" (see the dynamic label in updateSizingDropdownOptions()), so
      // shelfCount computed from it is already a PER-PAIR shelf count, not a cluster-wide total
      // to be divided among pairs -- EVERY pair gets its own full shelfCount shelves, numbered
      // sequentially across pairs (pair 1: Shelf A1..AshelfCount, pair 2: continues from there,
      // etc.) so no two pairs' shelf numbers ever collide. A single NS224 shelf has only 4
      // physical storage ports (2 per NSM), fully consumed by ONE HA pair's redundant multipath
      // wiring, so two independent HA pairs can never physically share one shelf -- dividing
      // shelfCount by numPairs (the prior bug here) starved every pair but the first of its
      // rightful shelves instead.
      const numPairs = Math.max(1, Math.ceil(halfNodes / 2));
      const sizingInfo = getExpansionCardsAndPorts(model, state.sizing.shelfType, shelfCount);

      // Site A shelves
      for (let pairIdx = 1; pairIdx <= numPairs; pairIdx++) {
        let nodeA, nodeB;
        if (halfNodes === 1) {
          nodeA = 1;
          nodeB = 1;
        ...
        const shelfStart = (pairIdx - 1) * shelfCount;

        for (let k = 0; k < shelfCount; k++) {
          const s = shelfStart + k + 1;
          const stackIdx = Math.floor(k / stackSize);
      ...
      // 2. Storage Cabling (shelves distributed across HA pairs). state.sizing.diskCount is
      // "Disks per Node Pair" (see the dynamic label in updateSizingDropdownOptions()), so
      // shelfCount computed from it is already a PER-PAIR shelf count, not a cluster-wide total
      // to be divided among pairs -- EVERY pair gets its own full shelfCount shelves, numbered
      // sequentially across pairs (pair 1: Shelf 1..shelfCount, pair 2: continues from there,
      // etc.) so no two pairs' shelf numbers ever collide. A single NS224 shelf has only 4
      // physical storage ports (2 per NSM), fully consumed by ONE HA pair's redundant multipath
      // wiring, so two independent HA pairs can never physically share one shelf -- dividing
      // shelfCount by numPairs (the prior bug here) starved every pair but the first of its
      // rightful shelves instead.
      const numPairs = Math.max(1, Math.ceil(nodeCount / 2));
      const sizingInfo = getExpansionCardsAndPorts(model, state.sizing.shelfType, shelfCount);
      ...
      for (let pairIdx = 1; pairIdx <= numPairs; pairIdx++) {
        ...
        for (let k = 0; k < shelfCount; k++) {
          const shelfNum = shelfStart + k + 1;
          const stackIdx = Math.floor(k / stackSize);
          const activePorts = sizingInfo.portPairs[stackIdx] || sizingInfo.portPairs[sizingInfo.portPairs.length - 1];

          rows.push({ src: getNodeName(nodeA), srcPort: activePorts[0], dest: `Shelf ${shelfNum} NSM A`, destPort: "Port e0a", type: `Storage HA Multipath A (${nodeA}->A)` });
          rows.push({ src: getNodeName(nodeA), srcPort: activePorts[1], dest: `Shelf ${shelfNum} NSM B`, destPort: "Port e0b", type: `Storage HA Multipath B (${nodeA}->B)` });
          if (nodeB <= nodeCount) {
            rows.push({ src: getNodeName(nodeB), srcPort: activePorts[0], dest: `Shelf ${shelfNum} NSM B`, destPort: "Port e0a", type: `Storage HA Multipath A (${nodeB}->B)` });
            rows.push({ src: getNodeName(nodeB), srcPort: activePorts[1], dest: `Shelf ${shelfNum} NSM A`, destPort: "Port e0b", type: `Storage HA Multipath B (${nodeB}->A)` });
          }
        }
      }
...
function generateOntapCliCode() {
  ...
  if (state.mode === "greenfield") {
    ...
    // generateCablingRows()), so this PCIe expansion-card requirement is the same for every node
    // regardless of which pair it's in or whether MetroCluster is enabled -- it must not be
    // divided further by the number of pairs.
    const sizingInfo = getExpansionCardsAndPorts(model, shelfType, shelfCount);
    const ports = getControllerPorts(model);
    ...
    if (isAsaR2) {
      ...
      // One aggregate per node, across every HA pair -- not just the first pair. diskCount is
      // already per-pair, so halfDiskCount (that pair's disks split across its 2 nodes) is the
      // same for every pair.
      for (let p = 1; p <= nodeCount / 2; p++) {
        const pairNodeA = state.customNodeNames[2 * p - 2] || `cluster1-0${2 * p - 1}`;
        const pairNodeB = state.customNodeNames[2 * p - 1] || `cluster1-0${2 * p}`;
        code += `storage aggregate create -aggregate ${aggrPrefix}_${2 * p - 1} -node ${pairNodeA} -diskcount ${halfDiskCount} -raidtype ${state.sizing.raidType} -raidgroupsize ${state.sizing.raidGroupSize}${mirrorParam}\n`;
        code += `storage aggregate create -aggregate ${aggrPrefix}_${2 * p} -node ${pairNodeB} -diskcount ${halfDiskCount} -raidtype ${state.sizing.raidType} -raidgroupsize ${state.sizing.raidGroupSize}${mirrorParam}\n`;
      }
      code += `\n`;
    }
  }
  ...
  state.svms.forEach(svm => {
    if (state.mode === "greenfield" || !svm.fromAsup) {
      ...
      if (versionNum >= 917) {
        // JIT (Just-In-Time) privilege elevation, ONTAP 9.17.1+: a custom least-privilege role,
        // temporarily elevated to on demand via "security jit-privilege", not a login-time flag.
        code += `security login role create -role jit_admin -cmddirname "volume" -access all -vserver ${svm.name}\n`;
        code += `security jit-privilege user create -username admin -vserver ${svm.name} -role jit_admin -session-validity-period 1h\n`;
      }
      code += `\n`;
    } else {
  ...
  // Ethernet-based LIFs (NFS/SMB/iSCSI/NVMe-TCP/S3) home to this platform's real data ports
  // (see getControllerPorts()/DATA_SOURCES.md) so the CLI matches the cabling diagram.
  const lifPorts = getControllerPorts(state.sizing.controller);
  ...
  state.svms.forEach(svm => {
    ...
    if (proto === "nfs") {
      code += `network interface create -vserver ${svm.name} -lif lif_nfs_${svm.name}_1 ${policyNas} -home-node ${node1} -home-port ${lifPorts.data[0]}${vlanStr} -address ${ip1} -netmask 255.255.255.0\n`;
      code += `network interface create -vserver ${svm.name} -lif lif_nfs_${svm.name}_2 ${policyNas} -home-node ${node2} -home-port ${lifPorts.data[1]}${vlanStr} -address ${ip2} -netmask 255.255.255.0\n`;
    } else if (proto === "smb") {
      code += `network interface create -vserver ${svm.name} -lif lif_smb_${svm.name}_1 ${policySmb} -home-node ${node1} -home-port ${lifPorts.data[0]}${vlanStr} -address ${ip1} -netmask 255.255.255.0\n`;
      code += `network interface create -vserver ${svm.name} -lif lif_smb_${svm.name}_2 ${policySmb} -home-node ${node2} -home-port ${lifPorts.data[1]}${vlanStr} -address ${ip2} -netmask 255.255.255.0\n`;
    } else if (proto === "iscsi") {
      code += `network interface create -vserver ${svm.name} -lif lif_iscsi_${svm.name}_1 ${policyIscsi} -home-node ${node1} -home-port ${lifPorts.data[0]}${vlanStr} -address ${ip1} -netmask 255.255.255.0\n`;
      code += `network interface create -vserver ${svm.name} -lif lif_iscsi_${svm.name}_2 ${policyIscsi} -home-node ${node2} -home-port ${lifPorts.data[1]}${vlanStr} -address ${ip2} -netmask 255.255.255.0\n`;
      ...
      code += `network interface create -vserver ${svm.name} -lif lif_nvme_tcp_${svm.name}_1 ${policyNvmeTcp} -home-node ${node1} -home-port ${lifPorts.data[0]}${vlanStr} -address ${ip1} -netmask 255.255.252.0 -data-protocol nvme-tcp\n`;
      code += `network interface create -vserver ${svm.name} -lif lif_nvme_tcp_${svm.name}_2 ${policyNvmeTcp} -home-node ${node2} -home-port ${lifPorts.data[1]}${vlanStr} -address ${ip2} -netmask 255.255.252.0 -data-protocol nvme-tcp\n`;
      ...
      code += `network interface create -vserver ${svm.name} -lif lif_s3_${svm.name}_1 ${policyS3} -home-node ${node1} -home-port ${lifPorts.data[0]} -address ${ip1} -netmask 255.255.252.0\n`;
    }
    code += `\n`;
  });
  ...
  if (isAsaR2) {
    ...
    state.volumes.forEach((vol, idx) => {
      ...
      const aggrName = state.mode === "greenfield" ? ((state.sizing.aggrNamePrefix || "aggr_data") + "_" + ((idx % nodeCount) + 1)) : vol.aggregate;
  ...
  activeProtos.forEach(p => {
    ...
    if (p === "iscsi") {
      ...
      state.svms.forEach(svm => {
        // igroup/lun create/lun map are unchanged on ASA r2 -- only "volume create" and
        // "storage aggregate create" are unsupported there (see DATA_SOURCES.md); lun create
        // auto-provisions its containing volume in the Storage Availability Zone on ASA r2.
        if (isAsaR2) {
          code += `# [ASA r2] iSCSI configuration for ${svm.name} (lun create auto-provisions its volume in the SAZ)\n`;
        } else {
        ...
        // Create (ASA r2 only) and map LUNs for each volume
        state.volumes.filter(v => v.svmName === svm.name && (v.type === "san" || (v.luns && v.luns.length > 0))).forEach(vol => {
          ...
          vol.luns.forEach(lun => {
            if (!lun.name.startsWith("ns_")) {
              const path = `/vol/${vol.name}/${lun.name}`;
              if (isAsaR2) {
                code += `lun create -vserver ${svm.name} -path ${path} -size ${lun.size}${lun.sizeUnit} -ostype ${lun.osType}\n`;
              }
    ...
    if (p === "fc" || p === "fcoe") {
      ...
      state.svms.forEach(svm => {
        // igroup/lun create/lun map are unchanged on ASA r2 -- see the iSCSI section above
        // and DATA_SOURCES.md for why (only volume/aggregate create are unsupported there).
        if (isAsaR2) {
          code += `# [ASA r2] FCP configuration for ${svm.name} (lun create auto-provisions its volume in the SAZ)\n`;
        } else {
    ...
    if (p === "nvme_tcp" || p === "nvme_fc") {
      ...
      state.svms.forEach(svm => {
        // vserver nvme subsystem/namespace commands are unchanged on ASA r2 -- see the iSCSI
        // section above and DATA_SOURCES.md for why (only volume/aggregate create are
        // unsupported there; namespace create auto-provisions its volume in the SAZ).
        if (isAsaR2) {
          code += `# [ASA r2] NVMe configuration for ${svm.name} (namespace create auto-provisions its volume in the SAZ)\n`;
        } else {
        ...
        // Create (ASA r2 only) and map Namespaces for each volume
        state.volumes.filter(v => v.svmName === svm.name && (v.type === "san" || (v.luns && v.luns.length > 0))).forEach(vol => {
          if (!vol.luns || vol.luns.length === 0) {
          ...
          vol.luns.forEach(lun => {
            if (lun.name.startsWith("ns_")) {
              const path = `/vol/${vol.name}/${lun.name}`;
              if (isAsaR2) {
                code += `vserver nvme namespace create -vserver ${svm.name} -path ${path} -size ${lun.size}${lun.sizeUnit} -ostype ${lun.osType}\n`;
              }
...
`;
  ...
  // Greenfield Aggregates Setup -- one aggregate per node, across every HA pair, not just
  // the first pair. diskCount is already per-pair, so halfDiskCount is the same for every pair.
  if (state.platform === "ontap" && state.mode === "greenfield") {
    const aggrPrefix = state.sizing.aggrNamePrefix || "aggr_data";
    const halfDiskCount = Math.floor(state.sizing.diskCount / 2);
    const raidTypeStr = state.sizing.raidType === "raid_dp" ? "raid_dp" : "raid_tec";

    for (let p = 1; p <= nodeCount / 2; p++) {
      const pairNodeA = state.customNodeNames[2 * p - 2] || `cluster1-0${2 * p - 1}`;
      const pairNodeB = state.customNodeNames[2 * p - 1] || `cluster1-0${2 * p}`;
      code += `    - name: Create Aggregate ${aggrPrefix}_${2 * p - 1} on Node ${2 * p - 1}
      netapp.ontap.na_ontap_aggregate:
        state: present
        name: "${aggrPrefix}_${2 * p - 1}"
        nodes: "${pairNodeA}"
        disk_count: ${halfDiskCount}
        raid_size: ${state.sizing.raidGroupSize}
        raid_type: "${raidTypeStr}"
        hostname: "{{ login.hostname }}"
        username: "{{ login.username }}"
        password: "{{ login.password }}"\n\n`;

      code += `    - name: Create Aggregate ${aggrPrefix}_${2 * p} on Node ${2 * p}
      netapp.ontap.na_ontap_aggregate:
        state: present
        name: "${aggrPrefix}_${2 * p}"
        nodes: "${pairNodeB}"
        disk_count: ${halfDiskCount}
        raid_size: ${state.sizing.raidGroupSize}
        raid_type: "${raidTypeStr}"
  ...
  state.volumes.forEach((vol, idx) => {
    ...
    code += `    - name: Create Volume ${vol.name}
      na_ontap_volume:
        ...
        compression: true${fpPolicyStr}${qosPolicyStr}
        hostname: "{{ login.hostname }}"
        username: "{{ login.username }}"
        password: "{{ login.password }}"\n\n`;
  });
...
function generateDeploymentGuide() {
  ...
  if (state.platform === "ontap") {
    ...
    // One aggregate per node (2 per HA pair) -- not a flat 2 aggregates cluster-wide.
    const totalAggregates = state.sizing.nodeCount;
    const disksPerAggr = state.sizing.diskCount / 2;
    ...
    const rawTotalParityDisks = parityDisksPerAggr * totalAggregates;
    const rawTotalSpareDisks = state.sizing.spareDisks * numPairs * 2;
    // Clamp parity + spares so the reported capacities can never exceed the physical raw capacity
    const totalParityDisks = Math.min(rawTotalParityDisks, totalDisks);
    const totalSpareDisks = Math.min(rawTotalSpareDisks, Math.max(0, totalDisks - totalParityDisks));
    const dataDisks = Math.max(0, totalDisks - totalParityDisks - totalSpareDisks);
...
function validateForm() {
  ...
  if (state.platform === "ontap") {
    ...
    if (state.mode === "greenfield") {
      ...
      // One aggregate per node (2 per HA pair) -- disksPerAggr is diskCount-per-pair split
      // across that pair's 2 nodes, independent of how many pairs the cluster has.
      const disksPerAggr = state.sizing.diskCount / 2;
      ...
      // Cabling shelf counts checks -- driven by the real per-platform PCIe expansion-slot
      // capacity in getExpansionCardsAndPorts() instead of a blanket ">2 shelves" rule. When
      // the configuration fits, the needed adapters are auto-provisioned into the BOM/CLI/
      ...
      if (shelfCardSizing.overflow) {
        // shelfCount/maxDirectAttachShelves here are external-shelf-only (see getShelfCount()) --
        // the real total disk-count ceiling this platform supports also includes any internal
        // bays (getInternalBayCount()), which are consumed first and don't need a shelf at all.
        const internalBaysForMsg = getInternalBayCount(state.sizing.controller) || 0;
        const realDiskMax = internalBaysForMsg + shelfCardSizing.maxDirectAttachShelves * getShelfBayCount(state.sizing.shelfType);
        const internalNote = internalBaysForMsg > 0 ? ` (including ${internalBaysForMsg} disks that need no shelf at all)` : "";
        errors.push({
          step: 7,
          title: "Shelf Count Exceeds Direct-Attach Capacity",
          msg: `${state.sizing.shelfType} Shelves (${shelfCount} external, beyond ${internalBaysForMsg} internal disks per HA pair): ${state.sizing.controller} can direct-attach at most ${shelfCardSizing.maxDirectAttachShelves} external ${state.sizing.shelfType} shelves per HA pair (1 onboard stack plus ${shelfCardSizing.slotsAvailable} PCIe expansion slot(s)).`,
          why: `Each expansion card adds one more storage stack of capacity, but ${state.sizing.controller} only has ${shelfCardSizing.slotsAvailable} usable slot(s) for this -- there's no physical slot left to cable the remaining shelves directly.`,
          fix: `Reduce Disk Count on Step 1 to ${realDiskMax} or fewer disks per node pair${internalNote}, or use a storage switch fabric topology to scale beyond onboard/PCIe direct-attach limits.`
        });
      }

      // NOTE: state.sizing.diskCount is "Disks per Node Pair" for ONTAP (see the dynamic label
      // in updateDiskSizeOptions()/updateSizingDropdownOptions()) -- shelfCount computed from it
      // is already a PER-PAIR shelf count, not a cluster-wide total. Every HA pair automatically
      // gets its own dedicated shelfCount shelves (see generateCablingRows()), so there is no
      // "insufficient shelves for node count" scenario to warn about here; a warning comparing
      // pair count against this per-pair shelfCount was comparing incompatible units and is
      // removed. See CHANGELOG.md for the full correction (shelvesPerPair must equal shelfCount,
      // never shelfCount/numPairs, everywhere that pattern appeared across the app).
        ...
        const unitLabel = isNvme ? "Namespace" : "LUN";
          ...
          vol.luns.forEach((lun, lunIdx) => {
            ...
            if (!lun.size || lun.size <= 0) {
              ...
              // Normalize to GB before summing -- LUNs and their parent volume can each
              // independently use GB or TB, and summing raw lun.size values ignores that.
              let lunSizeGb = lun.size;
              if (lun.sizeUnit === "TB") lunSizeGb *= 1000;
              totalLunSizeGb += lunSizeGb;
            }
          ...
          if (totalLunSizeGb > volSizeGb) {
            errors.push({
              step: 4,
              title: "Overprovisioned Volume Space",
              msg: `Volume "${vol.name}": Sum of nested ${unitLabel} sizes (${formatCapacity(totalLunSizeGb)}) exceeds parent volume capacity (${formatCapacity(volSizeGb)}).`,
          ...
          } else if (totalLunSizeGb >= volSizeGb * 0.95) {
            warnings.push({
              step: 4,
              title: "Insufficient Volume Headroom for LUNs",
              msg: `Volume "${vol.name}": Nested ${unitLabel}s consume ${((totalLunSizeGb / volSizeGb) * 100).toFixed(0)}% of containing volume capacity (${formatCapacity(volSizeGb)}).`,
...
function generateSvgPhysicalCabling() {
  ...
  if (state.platform === "ontap" && state.metrocluster && state.metrocluster.enabled) {
    ...
    const totalShelvesPerSite = shelfCount * numPairs;
    const sizingInfo = getExpansionCardsAndPorts(model, shelfType, shelfCount);
    ...
    const svgWidth = 1000;
    const svgHeight = storageStartY + totalShelvesPerSite * 60 + (hasFabricPool ? 120 : 40);
    ...
    // Site A Shelves
    for (let s = 1; s <= totalShelvesPerSite; s++) {
    ...
    for (let s = 1; s <= totalShelvesPerSite; s++) {
      const yOff = storageStartY + (s - 1) * 60;
      const pairIdx = Math.floor((s - 1) / shelfCount) + 1;
      let nodeA, nodeB;
      if (halfNodes === 1) {
        nodeA = 1;
        nodeB = 1;
      ...
      const shelfIdxWithinPair = (s - 1) % shelfCount;
  ...
  // Diagram is capped at the first 4 nodes (2 HA pairs) to avoid layout bloat;
  // shelves shown are scoped to those same visible pairs (each pair gets its
  // own full shelfCount, never shelfCount/numPairs — see CHANGELOG.md).
  const activeSvgNodes = Math.min(4, nodeCount);
  const activeSvgShelves = Math.ceil(activeSvgNodes / 2) * shelfCount;
  const sizingInfo = getExpansionCardsAndPorts(model, shelfType, shelfCount);
  ...
  for (let i = 0; i < activeSvgNodes; i++) {
    ...
    // Each node's cable lands on its own point along the destination switch's edge
    // (fanned out by node index) instead of every node converging on one fixed
    // coordinate, which drew as "every cable plugged into the same port."
    const fanFrac = (i + 0.5) / activeSvgNodes;
    if (isDirect) {
      if (i === 0) {
      ...
      const clusterSw1X = 240 + fanFrac * 150;
      const clusterSw2X = 410 + fanFrac * 150;
      svg += `  <path d="M 218 ${yOff + 42} C 218 ${yOff}, 260 50, ${clusterSw1X} 60" stroke="#00f2fe" stroke-width="2" fill="none" opacity="0.8"/>`;
      svg += `  <path d="M 248 ${yOff + 42} C 248 ${yOff}, 380 50, ${clusterSw2X} 60" stroke="#00f2fe" stroke-width="2" fill="none" opacity="0.8"/>`;
    }

    // Cables to management switch
    const mgmtEndY = mgmtY + fanFrac * 50;
    svg += `  <path d="M 378 ${yOff + 42} C 378 ${yOff + 70}, 160 ${mgmtEndY}, 150 ${mgmtEndY}" stroke="#9ca3af" stroke-width="1.5" stroke-dasharray="3,3" fill="none" opacity="0.7"/>`;

    // Cables to data switches
    const dataAEndY = dataAY + fanFrac * 50;
    const dataBEndY = dataBY + fanFrac * 50;
    svg += `  <path d="M 428 ${yOff + 42} C 428 ${yOff - 10}, 600 ${dataAEndY}, 650 ${dataAEndY}" stroke="#a855f7" stroke-width="2" fill="none" opacity="0.8"/>`;
    svg += `  <path d="M 458 ${yOff + 42} C 458 ${yOff + 70}, 600 ${dataBEndY}, 650 ${dataBEndY}" stroke="#a855f7" stroke-width="2" fill="none" opacity="0.8"/>`;
  }

  if (nodeCount > 4) {
    svg += `  <!-- Nodes note -->
    <g transform="translate(190, ${startY + activeSvgNodes * 80})">
      <text x="210" y="10" fill="rgba(255,255,255,0.4)" font-size="8">Note: Displaying first 4 nodes (${activeSvgNodes / 2} HA pair(s)) and their shelves out of the configured ${nodeCount}-node cluster. See Step 6 for the full cabling table.</text>
    </g>`;
  }
  ...
  for (let s = 1; s <= activeSvgShelves; s++) {
    ...
    // Each HA pair gets its own full shelfCount shelves (never shelfCount/numPairs)
    const pairIdx = Math.floor((s - 1) / shelfCount) + 1;
    const nodeA = 2 * pairIdx - 1;
    const nodeB = 2 * pairIdx;
...
function generateSvgStorageOnlyCabling() {
  ...
  if (state.platform === "ontap" && state.metrocluster && state.metrocluster.enabled) {
    ...
    const svgWidth = 960;
    const svgHeight = shelvesY + totalShelvesPerSite * 95 + 15;
...
function generateBillOfMaterials() {
  ...
  if (isSg) {
    ...
    // state.sizing.controller's format already varies by platform: ASA values are pre-prefixed
    // ("ASA_A90"), FAS values are plain ("FAS9500"), only AFF values ("A90") need "AFF-" added --
    // blindly prepending "AFF-" to all three produced nonsense part numbers like "AFF-ASA_A1K".
    let controllerPartNumber;
    if (state.ontapPlatform === "asa") {
      controllerPartNumber = state.sizing.controller.replace("ASA_", "ASA-");
    } else if (state.ontapPlatform === "afx") {
      controllerPartNumber = state.sizing.controller;
    } else {
      controllerPartNumber = `AFF-${state.sizing.controller}`;
    }
    bom += `| **Controller Pair** | High-Availability Controller Enclosure | ${controllerPartNumber} | ${nodeCount / 2} | HA Controller Pairs (dual-controller configurations). |\n`;
  }
  
  // Shelves
  if (!isSg || state.sizing.shelfType !== "virtual") {
    let shelfQty = 0;
    if (!isSg && (state.sizing.shelfType === "NS224" || state.sizing.shelfType === "DS224C" || state.sizing.shelfType === "DS212C")) {
      // ONTAP: route through getShelfCount() so the BOM agrees with the cabling/CLI/docs
      // generators, including internal-bay platforms where the first N disks per pair need
      // no external shelf at all (see getInternalBayCount()).
      shelfQty = getShelfCount(diskCount, state.sizing.shelfType, state.sizing.controller) * (nodeCount / 2);
    } else if (state.sizing.shelfType === "NS224" || state.sizing.shelfType === "DS224C") {
